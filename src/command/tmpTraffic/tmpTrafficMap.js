...
module.exports = async (ctx, cfg, serverName) => {
  ...
  // 查询地图玩家数据
  let mapData = await evmOpenApi.mapPlayerList(
    ctx.http,
    serverInfo.serverId,
    serverInfo.bounds[0][0],
    serverInfo.bounds[0][1],
    serverInfo.bounds[1][0],
    serverInfo.bounds[1][1],
  );
  ...
  for (const traffic of trafficData.data) {
    data.trafficList.push({
      ...
      province: await baiduTranslate(
        ctx,
        cfg,
        traffic.name.substring(0, traffic.name.lastIndexOf("(") - 1),
      ),
  ...
  try {
    ...
    await page.setViewport({ width: 999, height: 1033 });
    await page.goto(
