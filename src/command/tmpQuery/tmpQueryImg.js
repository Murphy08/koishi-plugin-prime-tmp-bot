...
module.exports = async (ctx, cfg, session, tmpId) => {
  ...
  data.registerDate = dayjs(playerInfo.data.registerTime).format(
    "YYYY年MM月DD日",
  );
  data.avatarUrl = playerInfo.data.avatarUrl;
  data.groupColor = playerInfo.data.groupColor;
  data.groupName =
    userGroup[playerInfo.data.groupName] || playerInfo.data.groupName;
  data.isJoinVtc = playerInfo.data.isJoinVtc;
  data.vtcName = playerInfo.data.vtcName;
  data.vtcId = playerInfo.data.vtcId;
  data.vtcJoinTime = playerInfo.data.vtcJoinTime;
  ...
  data.lastOnlineTime = playerInfo.data.lastOnlineTime;
  data.isOnline = false;
  if (playerMapInfo && !playerMapInfo.error) {
    ...
    if (data.isOnline) {
      ...
      data.onlineCountry = await baiduTranslate(
        ctx,
        cfg,
        playerMapInfo.data.location.poi.country,
      );
      data.onlineCity = await baiduTranslate(
        ctx,
        cfg,
